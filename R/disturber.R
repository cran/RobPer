disturber <-
function(tt,y,s,ps, s.outlier.fraction=0, interval){
if(interval & (max(tt)-3*ps)<0) stop("Light curve is too short to allow interval=TRUE")
n.points<- length(tt)
if(s.outlier.fraction>0){  hnr   <-sample((1:n.points), round(n.points*s.outlier.fraction), replace=FALSE)
                           s[hnr]<- 0.5*min(s)
                           }
if(interval==TRUE){ peak.start<-sample(tt[which(tt< (max(tt)-3*ps))],1)
