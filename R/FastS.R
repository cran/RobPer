...
function(x, y, Scontrol=list(int=FALSE, N=100, kk=2, tt=5, b=.5, cc=1.547,seed=NULL), beta_gamma) {
    #
...
int       <- ifelse(is.null(Scontrol$int)   ,FALSE,Scontrol$int) 
N         <- ifelse(is.null(Scontrol$N)     ,100,  Scontrol$N) 
kk        <- ifelse(is.null(Scontrol$kk)     ,2,    Scontrol$kk)
tt        <- ifelse(is.null(Scontrol$tt)    ,5,    Scontrol$tt)
b         <- ifelse(is.null(Scontrol$b)     ,.5,   Scontrol$b)
cc        <- ifelse(is.null(Scontrol$cc)    ,1.547,Scontrol$cc)
...
for(i in 1:N) {
  #Generate candidates: 
  if(!missing(beta_gamma)& i==1){ beta<- beta_gamma
      }else{
    ...
    for(i in tt:1) {
        tmp <- re.s(x=x,y=y,initial.beta=best.betas[i,],
                initial.scale=best.scales[i],kk=0,conv=1,b=b,cc=cc)
