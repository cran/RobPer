RobPer <-
function(ts, weighting, periods, regression, model, steps=10, tol=1e-3, var1=weighting, genoudcontrol=list(pop.size=50, max.generations=50,  wait.generations=5), LTSopt=TRUE, taucontrol=list(N=100, kk=2, tt=5, rr=2, approximate=FALSE), Scontrol=list(N=ifelse(weighting,200,50), kk=2, tt=5, b=.5, cc=1.547, seed=NULL)){ 
    ...
    # Check input 
        ...
        # taucontrol
            ...
            # check input
              ...
              if(any(!sapply(taucontrol[which(names(taucontrol)%in% c("N", "kk", "tt", "rr"))], is.wholenumber))) stop("List entries N, kk, tt and rr in taucontrol have to be positive integers")
              if(any(taucontrol[which(names(taucontrol)%in% c("N", "kk", "tt", "rr"))]<=0)) stop("List entries N, kk, tt and rr in taucontrol have to be strictly positive")     
              if(!is.null(taucontrol$approximate)){if(!is.logical(taucontrol$approximate)) stop("taucontrol$approximate is not logical")}           
           }
        ...
        # Scontrol
            ...
            # check list and fill it up
              Scontrol$int<-                                  FALSE
              if(is.null(Scontrol$N))      Scontrol$N<-       ifelse(weighting,200,50)  
            ...
            # check input
              ...
              if(any(!sapply(Scontrol[which(names(Scontrol)%in% c("N", "kk", "tt", "b","cc"))], is.numeric))) stop("List entries N, kk, tt, b and cc in Scontrol have to be numeric")     
              if(any(!sapply(Scontrol[which(names(Scontrol)%in% c("N", "kk", "tt"))], is.wholenumber))) stop("List entries N, kk and tt in Scontrol have to be positive integers")
              if(any(Scontrol[which(names(Scontrol)%in% c("N", "kk", "tt", "b","cc"))]<=0)) stop("List entries N, kk,tt, b and cc in Scontrol have to be strictly positive")               
           }        

            
        # Clean time series: Delete incomplete observations 
            lookatcol<- ifelse(weighting,3,2)
            if(any(is.na(ts[,1:lookatcol]))) message("There exist incomplete observations which are ignored.")
            if(weighting)  ts<- ts[complete.cases(ts),] 
            if(!weighting) ts<- ts[complete.cases(ts[,1:2]),] 
        ...
        if(regression=="LTS" & !(design %in%c("step", "stepB"))){ 
                    ...
                    if(length(unique(ii))==1){tempINT  <- ltsReg(yy~1  , use.correction=FALSE, alpha=alpha)}else{                                  
        ...
        if(regression %in% c("huber", "bisquare")){
                 singleFUN<- function(p_ind, design){
                      singlePerM(tt=tt,yy=yy,s=s,n=n,ii=ii, pp=periods[p_ind],W=W,rho=rho,regression=regression, design=design, steps=steps, var1=var1, tol=tol, genoudcontrol=genoudcontrol)
                      }
          }else{
                singleFUN<- function(p_ind, design){
                      singlePernotM(tt=tt,yy=yy,s=s,n=n,ii=ii,pp=periods[p_ind], design=design, regression=regression, steps=steps, zeta=zeta, LTSopt=LTSopt, tol=tol, genoudcontrol=genoudcontrol, gamma=gamma, K_gamma=K_gamma, taucontrol=taucontrol, Scontrol=Scontrol)
                      }        
          }
