singlePernotM <-
function(tt,yy,s,n,ii, pp, regression, design, steps, zeta, LTSopt, tol,  genoudcontrol,  gamma, K_gamma, taucontrol, Scontrol){
    # Used by RobPer 
  ...
  if(is.na(XX[1])){ return(NA)}
  if(dim(XX)[2]==1){ return(0)} # In case of only one step to fit
  ...
  if(regression=="LTS"){
   ...
   #Full model
    ...
    if(all(XX[,1]==1)){  tempFULL<-try(tempFULL<-ltsReg(yy~1+XX[,-1], mcd=FALSE, alpha=alpha), silent=TRUE) #ltsReg seldomly crashes... give it a second chance
                         if(inherits(tempFULL, "try-error")){tempFULL<-try(ltsReg(yy~1+XX[,-1], mcd=FALSE, alpha=alpha), silent=TRUE)} #... and a third time
                         if(inherits(tempFULL, "try-error")){tempFULL<-try(ltsReg(yy~1+XX[,-1], mcd=FALSE, alpha=alpha), silent=TRUE)}
                         ...
                         if(inherits(tempFULL, "try-error")){ tempFULL<-try(ltsReg(yy~0+XX, mcd=FALSE, alpha=alpha), silent=TRUE)} #... and a third time
                         if(inherits(tempFULL, "try-error")){ tempFULL<-try(ltsReg(yy~0+XX, mcd=FALSE, alpha=alpha), silent=TRUE)}
                         if(inherits(tempFULL, "try-error")){  LTSproblem<-TRUE} # if still problems, skip this period.
                         }
   if(LTSproblem){if(LTSopt){tempFULL<-suppressWarnings(rq(yy~0+XX, tau=.5, method="br"))
                             }else{ 
                             return(NA)
   ...
   # Use genetic optimization
    if(LTSopt==TRUE){
        ...
        beta_<-suppressWarnings(genoud(optf, BFGS=FALSE, nvars=length(beta_),starting.values=rbind(beta_,beta_gamma ), pop.size=genoudcontrol$pop.size, max=FALSE, max.generations=genoudcontrol$max.generations, wait.generations=genoudcontrol$wait.generations, solution.tolerance=tol, print.level=0)$par)
        rr<- as.vector(yy-XX%*%beta_)
        }    
    if(LTSopt==FALSE){
     rr<- tempFULL$residuals
    }
